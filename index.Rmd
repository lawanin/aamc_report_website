...
```{r revenue_source, warning = FALSE, message=FALSE, layout = "l-body-outset"}
...
up 51% of Tufts'. Of all AAMC schools, the average percentage received from tuition is 13%.

Federal grants contribute most to Harvard's total revenue, making up
47%. Of all AAMC schools, Harvard receives the second highest percentage of its total
revenue, 89%, from both kinds of grants combined, behind Mayo Clinic. The average percentage received from all grants is
roughly 30%. 
...
a %>% 
  ...
  mutate(total_grants = federal_grants + other_grants) %>% 
  select(school, tuition, federal_grants, other_grants, total_grants, support, hospital, gifts_endowment) -> a_percent
...
a_percent_avg <- a_percent %>% 
  ...
  select(-`1`) %>% 
  mutate(school = "Average") %>% 
  select(school, tuition, federal_grants, other_grants, total_grants, support, hospital, gifts_endowment) %>% 
...
```{r grants, warning = FALSE, message=FALSE, layout = "l-body-outset"}
...
Note that percent recorded and percent unrecorded should add to 100; likewise
...
Harvard exceeds the private school average in state and parent support. Harvard 
recieves no money from any other form of support. 
...
```{r hospital_programs, warning = FALSE, message=FALSE, layout = "l-body-outset"}
...
```{r aamc_endowment, warning = FALSE, message=FALSE, layout = "l-body-outset"}
...
Harvard has the most clinical faculty and most total faculty. Harvard has the 7th most preclincial 
faculty.
